 ...
 * TILE PALETTE 
 *
 * I just need to read a config file, then load all the tiles in that order. 
 * So, reading a file. First step. 
 *
 */
...
typedef struct {
	...
	NYFW_Rect color_rects[16];	// for the color picker tiles (WILL DELETE)

	NYFW_Canvas tile_canv;	// all the tiles
